#ifndef BRAFT_LOG_ENCRYPTED2_H
#define BRAFT_LOG_ENCRYPTED2_H
...
class BAIDU_CACHELINE_ALIGNMENT EncryptedSegmentTest 
        : public butil::RefCountedThreadSafe<EncryptedSegmentTest> {
public:
    EncryptedSegmentTest(const std::string& path, const int64_t first_index, const int64_t last_index)
        : _path(path), _bytes(0),
        _fd(-1), _is_open(true),
...
private:
friend class butil::RefCountedThreadSafe<EncryptedSegmentTest>;
    ~EncryptedSegmentTest() {
        if (_fd >= 0) {
            ::close(_fd);
            _fd = -1;
        }
    ...
    int _load_whole_entry(off_t offset, EntryHeader *head, butil::IOBuf *body, 
                    size_t size_hint) const;
                    
    //- Given iobuf, extract header info 
    //- (iv, log_index/term/type, mac, log_length)
    //- return value is log_length            
    int _extract_log_entry_header(butil::IOBuf read_iobuf, unsigned char* iv, unsigned char* aad, unsigned char* mac, EntryHeader* head) const;
...
class EncryptedSegmentTestLogStorage : public LogStorage {
public:
    typedef std::map<int64_t, scoped_refptr<EncryptedSegmentTest> > SegmentMap;

    explicit EncryptedSegmentTestLogStorage(const std::string& path, bool enable_sync = true)
        : _path(path)
    ...
    EncryptedSegmentTestLogStorage()    
        //- "-1" Indicates no valid counter
        : _log_counter_id(-1) 
        , _first_log_index(1)
        , _last_log_index(0)
    ...
    virtual ~EncryptedSegmentTestLogStorage() {}
    ...
    void sync();
private:
    scoped_refptr<EncryptedSegmentTest> open_segment();
    ...
    int get_segment(int64_t log_index, scoped_refptr<EncryptedSegmentTest>* ptr);
    void pop_segments(
            int64_t first_index_kept, 
            std::vector<scoped_refptr<EncryptedSegmentTest> >* poped);


    std::string _path;
    //- Deal with state-continuity
    butil::CounterID _log_counter_id;
    butil::atomic<int64_t> _first_log_index;
    butil::atomic<int64_t> _last_log_index;
    raft_mutex_t _mutex;
    SegmentMap _segments;
    scoped_refptr<EncryptedSegmentTest> _open_segment;
    bool _enable_sync;
};
