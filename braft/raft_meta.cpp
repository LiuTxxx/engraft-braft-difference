...
butil::Status StateContinuousMetaStorage::init() {
    butil::Status status;
    if (_is_inited) {
        return status;
    }
...
butil::Status StateContinuousMetaStorage::set_term_and_votedfor(const int64_t term, 
            const PeerId& peer_id, const VersionedGroupId& group) {
...
butil::Status StateContinuousMetaStorage::get_term_and_votedfor(int64_t* term, 
                                PeerId* peer_id, const VersionedGroupId& group) {
    butil::Status status;
    (void) group;
    if (!_is_inited) {
...
butil::Status StateContinuousMetaStorage::gc_instance(const std::string& uri, 
                                        const VersionedGroupId& vgid) const {
    butil::Status status;
    (void) uri;
    (void) vgid;
