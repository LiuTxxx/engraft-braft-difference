...
DEFINE_bool(use_distributed_counters, false, "Use distributed counters or not");
...
int ProtoBufFile::save(const google::protobuf::Message* msg, 
                    bool sync, butil::CounterID* counter_id, int flag) {
    std::string tmp_path(_path);
    tmp_path.append(".tmp");
    ...
    //- Get counter value
    butil::MonoCounterManager& manager = butil::GetGlobalMonoCntManager();
    LOG(INFO) << "Func: " << __FUNCTION__ << "CounterID = " << *counter_id;
    butil::CounterVal counter_value = 0;
    if (*counter_id == -1) {
        if (FLAGS_use_distributed_counters) {
            *counter_id = flag;
        } else {
            *counter_id =  manager.get_counter();
            LOG(INFO) << "Func: " << __FUNCTION__ << " REach here... in get counter";
        }
    }
    ...
    int32_t cipher_len = butil::gcm_encrypt(msg_plain, plaintext_len, aad, 12, key, iv, 12, msg_cipher, mac);
    timer.stop();
...
int ProtoBufFile::load(google::protobuf::Message* msg, 
                       butil::CounterID* counter_id, int flag) {
    butil::File::Error e;
    FileAdaptor* file = _fs->open(_path, O_RDONLY, NULL, &e);
    if (!file) {
        LOG(ERROR) << "open file failed, path: " << _path
    ...
    *counter_id = counter_index;
    ...
    //- Rollback detection
    if (butil::GetGlobalMonoCntManager().detect_rollback(counter_index, counter_value)) {
    ...
    unsigned char key[32];
    butil::get_sgx_seal_key_128(key);
    int decryptedtext_len = butil::gcm_decrypt(msg_cipher, cipher_len, aad, 12, mac, key, iv, 12, msg_plain);
