...
const static size_t ENC_ENTRY_HEADER_SIZE = 49;

struct EncryptedSegment::EntryHeader {
    int64_t index;
    int64_t term;
    int type;
    uint32_t data_len;
...
std::ostream& operator<<(std::ostream& os, const EncryptedSegment::EntryHeader& h) {
    os << "{term=" << h.term << ", type=" << h.type << ", data_len="
       << h.data_len << '}';
    return os;
}

int EncryptedSegment::create() {
    if (!_is_open) {
    ...
    //- Using fallocate brings little performance improvement but it fails the Raft recovery, so do not use it here
...
int EncryptedSegment::_load_entry(off_t offset, EntryHeader* head, butil::IOBuf* data,
                         size_t size_hint) const {
    if (size_hint > ENC_ENTRY_HEADER_SIZE) {
        return _load_whole_entry(offset, head, data, size_hint);
    }
    butil::IOPortal read_iobuf;
    const ssize_t n = file_pread(&read_iobuf, _fd, offset, ENC_ENTRY_HEADER_SIZE);    
    if (n != (ssize_t)ENC_ENTRY_HEADER_SIZE) {
        return n < 0 ? -1 : 1;
    }    
...
int EncryptedSegment::_extract_log_entry_header(butil::IOBuf read_iobuf, unsigned char* iv, unsigned char* aad, unsigned char* mac, EntryHeader* head) const {
    size_t buf_offset = 0;
    //- 1. Extract iv
...
int EncryptedSegment::_get_meta(int64_t index, LogMeta* meta) const {
...
int EncryptedSegment::_load_whole_entry(off_t offset, EntryHeader* head, butil::IOBuf* data,
                         size_t size_hint) const {
    ...
    int bytes_of_copied = read_iobuf.copy_to(msg_cipher, cipher_len, ENC_ENTRY_HEADER_SIZE);
...
int EncryptedSegment::load(ConfigurationManager* configuration_manager) {
    ...
    while (entry_off < file_size) {
        EntryHeader header;
        const int rc = _load_entry(entry_off, &header, NULL, ENC_ENTRY_HEADER_SIZE);
        if (rc > 0) {
        ...
        const int64_t skip_len = ENC_ENTRY_HEADER_SIZE + header.data_len;
...
int EncryptedSegment::append(const LogEntry* entry) {
    ...
    uint32_t cipher_len = butil::gcm_encrypt(data_plain, data_bytes, aad, 17, key, iv, 12, data_cipher, mac);
    // timer.stop();
...
int EncryptedSegment::sync(bool will_sync) {
...
LogEntry* EncryptedSegment::get(const int64_t index) const {
...
int64_t EncryptedSegment::get_term(const int64_t index) const {
    LogMeta meta;
    if (_get_meta(index, &meta) != 0) {
        return 0;
    }
...
std::string EncryptedSegment::file_name() {
...
int EncryptedSegment::truncate_prefix(const int64_t first_index_kept) {
...
int EncryptedSegment::truncate_suffix(const int64_t last_index_kept) {
...
int EncryptedSegmentLogStorage::init(ConfigurationManager* configuration_manager) {
...
int64_t EncryptedSegmentLogStorage::last_log_index() {
    return _last_log_index.load(butil::memory_order_acquire);
}

int EncryptedSegmentLogStorage::append_entries(const std::vector<LogEntry*>& entries, IOMetric* metric) {
    if (entries.empty()) {
        return 0;
    }
    ...
    scoped_refptr<EncryptedSegment> last_segment = NULL;
    int64_t now = 0;
    ...
    for (size_t i = 0; i < entries.size(); i++) {
        ...
        scoped_refptr<EncryptedSegment> segment = open_segment();
        ...
        //- Call EncryptedSegment::append func to conduct appending
    ...
    butil::Timer timer;
    // timer.start();
    //- Increase then store, so save meta firstly
    save_meta(first_log_index(), last_log_index());
    // timer.stop();
    // VLOG(79) << "TIME_OF calling save_meta = " << timer.m_elapsed(0) << " ms";
...
int EncryptedSegmentLogStorage::append_entry(const LogEntry* entry) {
    scoped_refptr<EncryptedSegment> segment = open_segment();
    if (NULL == segment) {
        return EIO;
    }
...
LogEntry* EncryptedSegmentLogStorage::get_entry(const int64_t index) {
    scoped_refptr<EncryptedSegment> ptr;
...
int64_t EncryptedSegmentLogStorage::get_term(const int64_t index) {
    scoped_refptr<EncryptedSegment> ptr;
    if (get_segment(index, &ptr) != 0) {
        return 0;
    }
...
void EncryptedSegmentLogStorage::pop_segments(
        const int64_t first_index_kept,
        std::vector<scoped_refptr<EncryptedSegment> >* popped) {
    popped->clear();
    popped->reserve(32);
    BAIDU_SCOPED_LOCK(_mutex);
    ...
    for (SegmentMap::iterator it = _segments.begin(); it != _segments.end();) {
        scoped_refptr<EncryptedSegment>& segment = it->second;
...
int EncryptedSegmentLogStorage::truncate_prefix(const int64_t first_index_kept) {
    ...
    LOG(INFO) << "Func: " << __FUNCTION__ << " EncryptedSegmentLogStorage calling save meta, first_index = " << first_index_kept;
...
int EncryptedSegmentLogStorage::truncate_suffix(const int64_t last_index_kept) {
    //- In -braft, only one segment (i.e., the _open_segment) is used
    if (_open_segment == NULL) {
        LOG(ERROR) << "Func: " << __FUNCTION__ << " Can't find open segment.";
        return -1;
...
int EncryptedSegmentLogStorage::reset(const int64_t next_log_index) {
    if (next_log_index <= 0) {
...
int EncryptedSegmentLogStorage::list_segments(bool is_empty) {
    ...
    while (dir_reader.Next()) {
        if (strcmp(dir_reader.name(), BRAFT_SEGMENT_OPEN_NAME) == 0) {
            ...
            //- Restore logs from the log data, later we will check consistency in EncryptedSegment::load
            else {
                //- The first/last index is extracted from log meta to 
                //- prevent rollback attack
                if (!_open_segment) {
                    _open_segment = new EncryptedSegment(_path, first_log_index(), last_log_index());
                } else {
...
int EncryptedSegmentLogStorage::load_segments(ConfigurationManager* configuration_manager) {
    int ret = 0;
    // open segment
    if (_open_segment) {
        ...
        //- fist/last_index in EncryptedSegmentLogStorage and _open_segment are the same.
    }
    //- TODO: Why?
...
int EncryptedSegmentLogStorage::save_meta(const int64_t first_log_index, 
...
int EncryptedSegmentLogStorage::load_meta() {
    butil::Timer timer;
    timer.start();
...
scoped_refptr<EncryptedSegment> EncryptedSegmentLogStorage::open_segment() {
    // scoped_refptr<EncryptedSegment> prev_open_segment;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        if (!_open_segment) {
...
int EncryptedSegmentLogStorage::get_segment(int64_t index, scoped_refptr<EncryptedSegment>* ptr) {
    BAIDU_SCOPED_LOCK(_mutex);
...
void EncryptedSegmentLogStorage::list_files(std::vector<std::string>* seg_files) {
    BAIDU_SCOPED_LOCK(_mutex);
...
void EncryptedSegmentLogStorage::sync() {
    std::vector<scoped_refptr<EncryptedSegment> > segments;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        for (SegmentMap::iterator it = _segments.begin(); it != _segments.end(); ++it) {
...
LogStorage* EncryptedSegmentLogStorage::new_instance(const std::string& uri) const {
    return new EncryptedSegmentLogStorage(uri);
}

butil::Status EncryptedSegmentLogStorage::gc_instance(const std::string& uri) const {
    butil::Status status;
    if (gc_dir(uri) != 0) {
