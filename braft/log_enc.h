...
class BAIDU_CACHELINE_ALIGNMENT EncryptedSegment 
        : public butil::RefCountedThreadSafe<EncryptedSegment> {
public:
...
private:
friend class butil::RefCountedThreadSafe<EncryptedSegment>;
    ~EncryptedSegment() {
        if (_fd >= 0) {
            ::close(_fd);
            _fd = -1;
...
public:
    typedef std::map<int64_t, scoped_refptr<EncryptedSegment> > SegmentMap;
    ...
    void sync();
private:
    scoped_refptr<EncryptedSegment> open_segment();
    ...
    int get_segment(int64_t log_index, scoped_refptr<EncryptedSegment>* ptr);
    void pop_segments(
            int64_t first_index_kept, 
            std::vector<scoped_refptr<EncryptedSegment> >* poped);
    ...
    scoped_refptr<EncryptedSegment> _open_segment;
    bool _enable_sync;
};
