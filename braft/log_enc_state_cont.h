...
#include "butil/state_cont/distri_state_mgr.h"
#include "butil/third_party/merklecpp/merklecpp.h"
...
class BAIDU_CACHELINE_ALIGNMENT EncryptedSCSegment 
        : public butil::RefCountedThreadSafe<EncryptedSCSegment> {
public:
...
private:
friend class butil::RefCountedThreadSafe<EncryptedSCSegment>;
    ~EncryptedSCSegment() {
        if (_fd >= 0) {
            ::close(_fd);
            _fd = -1;
...
public:
    typedef std::map<int64_t, scoped_refptr<EncryptedSCSegment> > SegmentMap;
...
private:
    scoped_refptr<EncryptedSCSegment> open_segment();
    ...
    int get_segment(int64_t log_index, scoped_refptr<EncryptedSCSegment>* ptr);
    void pop_segments(
            int64_t first_index_kept, 
            std::vector<scoped_refptr<EncryptedSCSegment> >* poped);
    ...
    scoped_refptr<EncryptedSCSegment> _open_segment;
    bool _enable_sync;

    //- State continuity related variables
    butil::DistributedStateManager* _state_mgr;
