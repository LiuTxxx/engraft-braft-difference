...
int EncryptedSegmentTest::_load_entry(off_t offset, EntryHeader* head, butil::IOBuf* data,
...
int EncryptedSegmentTest::_extract_log_entry_header(butil::IOBuf read_iobuf, unsigned char* iv, unsigned char* aad, unsigned char* mac, EntryHeader* head) const {
    size_t buf_offset = 0;
    //- 1. Extract iv
...
int EncryptedSegmentTest::_load_whole_entry(off_t offset, EntryHeader* head, butil::IOBuf* data,
                         size_t size_hint) const {
...
int EncryptedSegmentTestLogStorage::append_entries(const std::vector<LogEntry*>& entries, IOMetric* metric) {
    ...
    scoped_refptr<EncryptedSegmentTest> last_segment = NULL;
    int64_t now = 0;
    ...
    for (size_t i = 0; i < entries.size(); i++) {
        ...
        scoped_refptr<EncryptedSegmentTest> segment = open_segment();
...
LogEntry* EncryptedSegmentTestLogStorage::get_entry(const int64_t index) {
    scoped_refptr<EncryptedSegmentTest> ptr;
...
void EncryptedSegmentTestLogStorage::pop_segments(
        ...
        std::vector<scoped_refptr<EncryptedSegmentTest> >* popped) {
    popped->clear();
    popped->reserve(32);
    BAIDU_SCOPED_LOCK(_mutex);
    ...
    for (SegmentMap::iterator it = _segments.begin(); it != _segments.end();) {
        scoped_refptr<EncryptedSegmentTest>& segment = it->second;
...
scoped_refptr<EncryptedSegmentTest> EncryptedSegmentTestLogStorage::open_segment() {
    // scoped_refptr<EncryptedSegmentTest> prev_open_segment;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        if (!_open_segment) {
...
int EncryptedSegmentTestLogStorage::get_segment(int64_t index, scoped_refptr<EncryptedSegmentTest>* ptr) {
    BAIDU_SCOPED_LOCK(_mutex);
...
void EncryptedSegmentTestLogStorage::sync() {
    std::vector<scoped_refptr<EncryptedSegmentTest> > segments;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        for (SegmentMap::iterator it = _segments.begin(); it != _segments.end(); ++it) {
...
butil::Status EncryptedSegmentTestLogStorage::gc_instance(const std::string& uri) const {
    butil::Status status;
    if (gc_dir(uri) != 0) {
