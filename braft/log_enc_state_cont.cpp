#include "braft/log_enc_state_cont.h"
...
#include "butil/state_cont/openssl_utils.h"
...
// DEFINE_int32(log_merkle_tree_size, 512, "the size of merkle tree");
...
const static size_t ENC_ST_ENTRY_HEADER_SIZE = 49;

struct EncryptedSCSegment::EntryHeader {
    int64_t index;
    int64_t term;
    int type;
    uint32_t data_len;
...
std::ostream& operator<<(std::ostream& os, const EncryptedSCSegment::EntryHeader& h) {
    os << "{term=" << h.term << ", type=" << h.type << ", data_len="
       << h.data_len << '}';
    return os;
}

int EncryptedSCSegment::create() {
    if (!_is_open) {
    ...
    VLOG(79) << " Func: " << __FUNCTION__ << " path = " << path;
    _fd = ::open(path.c_str(), O_RDWR | O_CREAT | O_TRUNC, 0644);
    if (_fd < 0) {
        LOG(ERROR) << "Func: " << __FUNCTION__ << " Open file failed, "
            << " errno = " << berror();
    }
    // ocall_fallocate(_fd, 0, 1024*1024*1024);
    posix_fallocate(_fd, 0, 1024*1024*128);
...
int EncryptedSCSegment::_load_entry(off_t offset, EntryHeader* head, 
                                butil::IOBuf* data, size_t size_hint) const {
    if (size_hint > ENC_ST_ENTRY_HEADER_SIZE) {
        return _load_whole_entry(offset, head, data, size_hint);
    }
    butil::IOPortal read_iobuf;
    const ssize_t n = file_pread(&read_iobuf, _fd, offset, ENC_ST_ENTRY_HEADER_SIZE);    
    if (n != (ssize_t)ENC_ST_ENTRY_HEADER_SIZE) {
        return n < 0 ? -1 : 1;
    }    
    ...
    butil::get_sgx_seal_key_128(key);
    log_data_iobuf.copy_to(cipher_data, cipher_len);
    int decryptedtext_len = butil::gcm_decrypt(cipher_data, cipher_len, aad, 17, mac, key, iv, 12, plain_data);
    //- Check results
...
int EncryptedSCSegment::_extract_log_entry_header(butil::IOBuf read_iobuf, unsigned char* iv, unsigned char* aad, unsigned char* mac, EntryHeader* head) const {
    size_t buf_offset = 0;
    //- 1. Extract iv
...
int EncryptedSCSegment::_get_meta(int64_t index, LogMeta* meta) const {
...
int EncryptedSCSegment::_load_whole_entry(off_t offset, EntryHeader* head, butil::IOBuf* data,
    ...
    int bytes_of_copied = read_iobuf.copy_to(msg_cipher, cipher_len, ENC_ST_ENTRY_HEADER_SIZE);
    ...
    unsigned char key[32];
    butil::get_sgx_seal_key_128(key);
    int decryptedtext_len = butil::gcm_decrypt(msg_cipher, cipher_len, aad, 17, mac, key, iv, 12, msg_plain);
    //- Check results
    if (decryptedtext_len != cipher_len) {
        LOG(ERROR) << "Func: " << __FUNCTION__ << " Log decryption failed";
        return -1;
...
int EncryptedSCSegment::load(ConfigurationManager* configuration_manager) {
    ...
    while (entry_off < file_size) {
        EntryHeader header;
        const int rc = _load_entry(entry_off, &header, NULL, ENC_ST_ENTRY_HEADER_SIZE);
        if (rc > 0) {
        ...
        const int64_t skip_len = ENC_ST_ENTRY_HEADER_SIZE + header.data_len;
...
int EncryptedSCSegment::append(const LogEntry* entry) {
    BAIDU_SCOPED_LOCK(log_sum_mutex);
    ...
    //- Calculate hashes
    butil::Timer timer;
    LogHashSummary temp_log_hash_sum;
    if (butil::generate_sha256_hash(data_plain, data_bytes, temp_log_hash_sum.log_hash) != 0) {
        LOG(ERROR) << "Func: " << __FUNCTION__ << " Hash error #1!";
        return -1;
    }
    uint8_t two_hash[64];
    memcpy(two_hash, temp_log_hash_sum.log_hash, 32);
    memcpy(two_hash + 32, log_hash_summary, 32);
    if (butil::generate_sha256_hash(two_hash, 64, temp_log_hash_sum.log_hash_sum) != 0) {
        LOG(ERROR) << "Func: " << __FUNCTION__ << " Hash error #2!";
        return -1;
    }
    memcpy(log_hash_summary, temp_log_hash_sum.log_hash_sum, 32);

    {
        BAIDU_SCOPED_LOCK(log_map_mutex);
        if (log_hash_map.size() >= 1024) {
            log_hash_map.clear();
        }
        log_hash_map[entry->id.index] = temp_log_hash_sum;
    }
    

    //- Buffer for iv, key, mac
    unsigned char iv[12];
    butil::get_init_vector(iv, sizeof(iv));
    unsigned char key[32];
    butil::get_sgx_seal_key_128(key);
    unsigned char mac[16];
    //- Construct AAD
    ...
    int32_t cipher_len = butil::gcm_encrypt(data_plain, data_bytes, aad, 17, key, iv, 12, data_cipher, mac);
    // timer.stop();
    // VLOG(85) << "Func: " << __FUNCTION__ << " TIME_OF log encryption = "
    //     << timer.m_elapsed(0.0) << " ms";
    //- Then transform data_cipher to iobuf
    butil::IOBuf data_cipher_out;
    ...
    butil::IOBuf* pieces[2] = { &log_entry_header, &data_cipher_out };
    size_t start = 0;
    ssize_t written = 0;
    // timer.start();
...
int EncryptedSCSegment::sync(bool will_sync) {
...
LogEntry* EncryptedSCSegment::get(const int64_t index) const {
...
int64_t EncryptedSCSegment::get_term(const int64_t index) const {
    LogMeta meta;
    if (_get_meta(index, &meta) != 0) {
        return 0;
    }
...
std::string EncryptedSCSegment::file_name() {
...
int EncryptedSCSegment::truncate_prefix(const int64_t first_index_kept) {
...
int EncryptedSCSegment::truncate_suffix(const int64_t last_index_kept) {
...
int EncryptedSCSegmentLogStorage::init(ConfigurationManager* configuration_manager) {
    _state_mgr = butil::GetGlobalDistributedStateManager();
...
int64_t EncryptedSCSegmentLogStorage::last_log_index() {
    return _last_log_index.load(butil::memory_order_acquire);
}

int EncryptedSCSegmentLogStorage::append_entries(const std::vector<LogEntry*>& entries, IOMetric* metric) {
    if (entries.empty()) {
        return 0;
    }
    ...
    scoped_refptr<EncryptedSCSegment> last_segment = NULL;
    int64_t now = 0;
    ...
    for (size_t i = 0; i < entries.size(); i++) {
        ...
        scoped_refptr<EncryptedSCSegment> segment = open_segment();
        ...
        //- Call EncryptedSCSegment::append func to conduct appending
    ...
    now = butil::cpuwide_time_us();
    butil::Timer timer;
    //- Store then "increase": Put fsync in the first step, and then save log hash states
    timer.start();
    last_segment->sync(_enable_sync);
    save_meta(first_log_index(), last_log_index());
    timer.stop();
    // VLOG(79) << "Func: " << __FUNCTION__ << " TIME_OF log persistence #2: sync = "
...
int EncryptedSCSegmentLogStorage::append_entry(const LogEntry* entry) {
    scoped_refptr<EncryptedSCSegment> segment = open_segment();
    if (NULL == segment) {
        return EIO;
    }
...
LogEntry* EncryptedSCSegmentLogStorage::get_entry(const int64_t index) {
    scoped_refptr<EncryptedSCSegment> ptr;
...
int64_t EncryptedSCSegmentLogStorage::get_term(const int64_t index) {
    scoped_refptr<EncryptedSCSegment> ptr;
    if (get_segment(index, &ptr) != 0) {
        return 0;
    }
...
void EncryptedSCSegmentLogStorage::pop_segments(
        const int64_t first_index_kept,
        std::vector<scoped_refptr<EncryptedSCSegment> >* popped) {
    popped->clear();
    popped->reserve(32);
    BAIDU_SCOPED_LOCK(_mutex);
    ...
    for (SegmentMap::iterator it = _segments.begin(); it != _segments.end();) {
        scoped_refptr<EncryptedSCSegment>& segment = it->second;
...
int EncryptedSCSegmentLogStorage::truncate_prefix(const int64_t first_index_kept) {
    //- In -braft, only one segment (i.e., the _open_segment) is used
    ...
    // LOG(INFO) << "Func: " << __FUNCTION__ << " EncryptedSCSegmentLogStorage calling save meta, first_index = " << first_index_kept;
    VLOG(79) << "Func: " << __FUNCTION__ << " EncryptedSCSegmentLogStorage calling save meta, first_index = " << first_index_kept;
    _first_log_index.store(first_index_kept, butil::memory_order_relaxed);
...
int EncryptedSCSegmentLogStorage::truncate_suffix(const int64_t last_index_kept) {
    //- In -braft, only one segment (i.e., the _open_segment) is used
    if (_open_segment == NULL) {
        LOG(ERROR) << "Func: " << __FUNCTION__ << " Can't find open segment.";
        return -1;
    ...
    _last_log_index.store(last_index_kept, butil::memory_order_relaxed);
    //- After truncation, save meta
    VLOG(79) << "Func: " << __FUNCTION__ << " EncryptedSCSegmentLogStorage calling save meta, last_index_kept = " << last_index_kept;
    save_meta(first_log_index(), last_index_kept);
    return ret;
}

int EncryptedSCSegmentLogStorage::reset(const int64_t next_log_index) {
    if (next_log_index <= 0) {
    ...
    VLOG(79) << "Func: " << __FUNCTION__ << " EncryptedSCSegmentLogStorage calling save meta, next_log_index = " << next_log_index;
...
int EncryptedSCSegmentLogStorage::list_segments(bool is_empty) {
    ...
    while (dir_reader.Next()) {
        if (strcmp(dir_reader.name(), BRAFT_SEGMENT_OPEN_NAME) == 0) {
            ...
            //- Restore logs from the log data, later we will check consistency in EncryptedSCSegment::load
            else {
                //- The first/last index is extracted from log meta to 
                //- prevent rollback attack
                if (!_open_segment) {
                    _open_segment = new EncryptedSCSegment(_path, first_log_index(), last_log_index());
                } else {
...
int EncryptedSCSegmentLogStorage::load_segments(ConfigurationManager* configuration_manager) {
    int ret = 0;
    // open segment
    if (_open_segment) {
        ...
        //- fist/last_index in EncryptedSCSegmentLogStorage and _open_segment are the same.
    }
    //- "first_index = last_index + 1" means that the log storage is empty
...
int EncryptedSCSegmentLogStorage::save_meta(const int64_t first_log_index, 
                                 const int64_t last_log_index) {
    butil::Timer timer;
    int leave_num = 0;
    pthread_mutex_lock(&_open_segment->log_sum_mutex);
    std::string log_hash
        (reinterpret_cast< char const*>(_open_segment->log_hash_summary), 32);
    pthread_mutex_unlock(&_open_segment->log_sum_mutex);
    int ret = _state_mgr->set_log_store_hash(log_hash, first_log_index, last_log_index);
    PLOG_IF(ERROR, ret != 0) << "Fail to save meta";
    // VLOG(79) << " first_log_index = " << first_log_index << " last_log_index = " 
    //     << last_log_index;
    return ret;
    return 0;
}
...
int EncryptedSCSegmentLogStorage::load_meta() {
    //- TODO: Retrieve log meta from state manager
    _load_meta_executed = true;
    return -1;
}

scoped_refptr<EncryptedSCSegment> EncryptedSCSegmentLogStorage::open_segment() {
    // scoped_refptr<EncryptedSCSegment> prev_open_segment;
    {
        BAIDU_SCOPED_LOCK(_mutex);
        if (!_open_segment) {
...
int EncryptedSCSegmentLogStorage::get_segment(int64_t index, scoped_refptr<EncryptedSCSegment>* ptr) {
    BAIDU_SCOPED_LOCK(_mutex);
...
LogStorage* EncryptedSCSegmentLogStorage::new_instance(const std::string& uri) const {
    return new EncryptedSCSegmentLogStorage(uri);
}

butil::Status EncryptedSCSegmentLogStorage::gc_instance(const std::string& uri) const {
    butil::Status status;
    if (gc_dir(uri) != 0) {
